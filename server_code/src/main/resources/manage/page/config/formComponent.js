            ...
            <el-form class="formModel_form" ref="formRef" :model="form" :rules="rules" >
                ...
                <el-form-item prop="value"
                    ...
                    <file-upload
                            ...
                            action="file/upload"
                            :multiple="true"
                            :file-urls="form.value?form.value:''"
...
methods: {
    init(formId=null,formType='add',formNames='',row=null,table=null,statusColumnName=null,tips=null,statusColumnValue=null){
        ...
        if(formType == 'add'){
            ...
            for(let x in row){
                ...
                if(x=='value'){
                    this.form.value = row[x];
                    this.disabledForm.value = true;
                    continue;
                }
            }
            if(row){
    ...
    getInfo(){
        http.get(`config/info/${this.id}`).then(res=>{
    ...
    save(){
        if(this.form.value!=null) {
            this.form.value = this.form.value.replace(new RegExp(baseUrl,"g"),"");
        }
        let objcross;
        let crossUserId = ''
        let crossRefId = ''
        ...
        this.$refs.formRef.validate((valid)=>{
            ...
            if(crossUserId&&crossRefId){
                ...
                http.get('config/page',{
                    params:params
                }).then(res=>{
                    if(res.data.data.total>=crossOptNum){
                        return this.$message.error(this.crossTips)
                    }else{
                        http.post(`config/${!this.form.id ? "save" : "update"}`,this.form).then(res=>{
                            if(this.type == 'cross'){
                                //修改跨表数据
